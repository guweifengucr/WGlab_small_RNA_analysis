...
foreach my $sa (@sam) {
	...
	open (hand0,">$sa\_all/$sa\_all_17_smRNA\_distribution_relative_to_ALG3426$nt26\_using_trans_$stan.txt");
...
sub g26_stat {
	...
	open (hand1,"alg34_fog2_20_twice.txt") or die $!;
	while (<hand1>) {
		$_ =~ s/\s+$//;
		my ($n1,$n2)=split /\t/;
		${$alg}{"$n1\t$n2"}=1;
	...
	close hand1;
	print "average transcript size of ALG-3/4 target mRNA ",$transnt/$tranno,"\n";
	print "genomic C rate for ALG-3/4 26G target mRNA ", $c/$gent,"\n";
	print "genomic C sites for ALG-3/4 26G target mRNA $c\n";
}
...
sub analyze_smRNA_distribution_flanking_26G {
	...
	close hand1;
	my $cno=scalar keys %c; print "$sam has $cno ALG-3/4 26G genomic loci\n\n\n";
