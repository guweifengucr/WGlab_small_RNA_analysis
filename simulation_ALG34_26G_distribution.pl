...
open (hand1,">ALG34_26G_distribution_simulation_expression_$failrate\_$g26no.txt");
...
open (hand1,">ALG34_26G_distribution_simulation_motif_$failrate\_$g26no.txt");
...
sub assign_26G {
	...
	for my $i (1..$g26no) {
		...
		next unless $po <$genesize*$region || $po >$genesize*(1-$region);
		...
		for my $j (1..$spreadstep) {
			...
			while ($seqdown =~ /G/g) {
				...
				next unless $po+$gp-1 <$genesize*$region || $po+$gp-1 >$genesize*(1-$region);
				my $ran=rand (1);
				if ($ran >$failrate) {
					$po+=($gp-1);
					$result{$po}+=$read;
